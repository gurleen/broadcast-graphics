...
| Export | Contents |
|--------|----------|
| `export default` | `definePackage({ id, name, version, templates })` — live zod schemas + **lazy** `Render` / `Controls` / `PreviewControls` factories |
| `export const manifest` | Pure JSON metadata: `formatVersion`, `runtime` range, package info, per-template `defaults` / `fields` / `transition` / `jsonSchema` |
